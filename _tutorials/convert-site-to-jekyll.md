...
First, let's start with a grounding in the basics. Stripping a Jekyll site down to an extremely basic level will help clarify what happens in a Jekyll site. If you haven't already installed the jekyll gem,.
...
This is a Jekyll site at the most basic functional level. Here's what is happening:
  ...
  * Jekyll looks for files with (the two sets of dashed lines `---` like those in `index.md`) and processes the files (populating site variables, rendering any [Liquid](https://shopify.github.io/liquid/), and converting Markdown to HTML).
...
You can even set [default front matter tags](/docs/configuration/front-matter-defaults/) for pages, posts, or in your `_config.yml` file so that you don't have to specify the layout in the front matter variables. Anyways, setting defaults is beyond the scope of this tutorial, let's get back to work.

...
1. **Site variables**. Jekyll looks across your files and populates , such as `site`, `page`, `post`, and collection objects. (From these objects, Jekyll determines the values for permalinks, tags, categories, and other details.)

2. **Liquid**. Jekyll processes any [Liquid](https://github.com/Shopify/liquid) formatting in pages that contain . You can identify Liquid as follows:
...
5. **Files**. Jekyll writes the generated content into files in the [directory structure] in `_site`. Pages, posts, and collections get structured based on their setting. Directories that begin with `_` (such as `_includes` and `_data`) are usually hidden in the output.
